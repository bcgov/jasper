...
public sealed class TdKeycloakTokenService : IKeycloakTokenService, IDisposable
{
    ...
    public async Task<string> GetAccessTokenAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            if (_cache.TryGetValue<TokenCacheEntry>(CacheKey, out entry)
                && !IsExpiringSoon(entry, _keycloakOptions.CurrentValue))
            {
                return entry.AccessToken;
            }
            ...
            _logger.LogDebug("Caching new access token.");
            var expiresAt = ComputeExpiry(token);

            entry = new TokenCacheEntry(token.AccessToken, expiresAt);
