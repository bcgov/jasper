...
{
    public class TransitoryDocumentsService
    {
        ...
        public async Task<IEnumerable<FileMetadataDto>> ListSharedDocuments(string bearer, string locationId, string roomCode, string date)
        {
            ...
            try
            {
                ...
                var regionName = await _locationService.GetRegionName(matchingLocation.AgencyIdentifierCd);
                if (string.IsNullOrWhiteSpace(regionName))
                {
                    _logger.LogError("Region not found for locationId: {LocationId}", locationId);
                    throw new BadRequestException("Region not found.");
                }
                _logger.LogDebug("Region {Region} found for locationId: {LocationId}", regionName, locationId);

                return await _tdClient.SearchAsync(regionName, locationShortName, roomCode, date);
            }
