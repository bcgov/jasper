...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const documentsDetailsJson = ref<civilDocumentType[]>([]);
      const providedDocumentsDetailsJson = ref<
        civilReferenceDocumentJsonType[]
      >([]);
        ...
        fetch(
          ...
          .then((data) => {
            if (data) {
              ...
              documentsDetailsJson.value = data.document;
              providedDocumentsDetailsJson.value = data.referenceDocument;
              if (data.sealedYN == 'Y') {
                isSealed.value = true;
              }
              ExtractCaseInfo();
              if (
                ...
                rightPartiesInfo.value.length > 0 ||
      ...
      const ExtractCaseInfo = () => {
        ...
        for (const jParty of partiesJson.value) {
          ...
          if (partyInfo.leftRight == 'R') {
            rightPartiesInfo.value.push(partyInfo);
          } else {
            leftPartiesInfo.value.push(partyInfo);
          }
        }
        leftPartiesInfo.value = SortParties(leftPartiesInfo);
        rightPartiesInfo.value = SortParties(rightPartiesInfo);
      ...
      const reloadCaseDetails = () => {
        ...
        documentsDetailsJson.value.length = 0;
        providedDocumentsDetailsJson.value.length = 0;
