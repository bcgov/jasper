    ...
    <b-card body-class="px-0" v-else>
      ...
      <b-form class="search-form" v-if="isLookupDataMounted && isLookupDataReady" @submit.prevent="handleSubmit">
        ...
        <b-form-group label="Division*" class="mr-3">
          <b-button-group>
            <b-button value="isCriminal" @click="handleDivisionChange" :variant="searchCriteria.isCriminal ? 'primary' : 'outline-primary'
              ">
              Criminal
            </b-button>
            <b-button value="isFamily" @click="handleDivisionChange" :variant="searchCriteria.isFamily ? 'primary' : 'outline-primary'
              ">
              Family
            </b-button>
            <b-button value="isSmallClaims" @click="handleDivisionChange" :variant="searchCriteria.isSmallClaims ? 'primary' : 'outline-primary'
              ">
              Small Claims
            </b-button>
          </b-button-group>
...
export default defineComponent({
  ...
  data() {
    ...
    return {
      ...
      defaultLocation:
        commonStore.userInfo.agencyCode,
      classOptions,
      searchCriteria,
      errors: {
  ...
  methods: {
    ...
    loadDataFromState(): void {
      const searchCriteria = this.courtFileSearchStore.currentSearchCriteria();
