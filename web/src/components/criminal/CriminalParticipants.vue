    ...
    <b-card no-body class="mx-3 mb-5">
      ...
      >
        ...
        <template v-for="(field, index) in fields" v-slot:[`cell(${field.key})`]="data">
          <b-badge class="mt-1" :style="data.field.cellStyle" variant="white" v-bind:key="index">
            {{ data.value }}
          </b-badge>
        </template>
        ...
        <template v-slot:cell(name)="data">
          <b-dropdown size="sm" style="height:35px;" no-caret variant="text-info">
            ...
            <b-dropdown-item-button disabled v-for="(file, index) in data.item.charges" :key="index">
  ...
  fields = [
    {
      ...
      cellStyle: "transform: translate(-10px,-4px); border:0px; font-size:16px",
    },
    {
      key: "dob",
      label: "D.O.B.",
